...
The docs site lives in [docs/](docs/) and is published to GitHub Pages. Start with [Flashing](docs/docs/getting-started/flashing.md) and [First boot](docs/docs/getting-started/first-boot.md), then [Writing](docs/docs/using/writing.md), [Git Sync](docs/docs/using/git-sync.md), [SFTP](docs/docs/using/sftp.md), [Disk Mode](docs/docs/using/disk-mode.md), [Settings](docs/docs/using/settings.md) and [Status LED](docs/docs/using/led.md). The reference section covers [Configuration](docs/docs/reference/configuration.md), [Hardware](docs/docs/reference/hardware.md) (which GPIO does what) and [Architecture](docs/docs/reference/architecture.md) (how the firmware is put together and why).
...
The emulator runs the same app core the device does. It reads key tokens from stdin, prints each frame to the terminal, and writes a PNG of it to `emu_frames/`. Every screenshot in the docs came out of it; `sim/screenshots.sh` regenerates the lot. `make test` runs the Git sync test against a local bare repository and the LCD packing test.
