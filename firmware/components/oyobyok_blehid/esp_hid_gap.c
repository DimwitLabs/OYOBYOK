...
static void handle_ble_device_result(const struct ble_gap_disc_desc *disc)
{
    ...
    // DEBUG (bring-up): log every advertisement seen, so we can tell "keyboard not advertising"
    // from "scan sees nothing". Prints addr, rssi, name, appearance, #uuids.
    ESP_LOGI(TAG, "adv %02x:%02x:%02x:%02x:%02x:%02x rssi=%d name='%s' appear=0x%04x uuids16=%d",
             disc->addr.val[5],disc->addr.val[4],disc->addr.val[3],
             disc->addr.val[2],disc->addr.val[1],disc->addr.val[0],
             disc->rssi, adv_name_len?(char*)adv_name:"", fields.appearance, fields.num_uuids16);
    ...
    // Accept ANY device advertising the HID service UUID (0x1812). The upstream example only
    // matched Espressif's own demo names ("ESP Keyboard" etc.), which rejected all real keyboards.
    bool is_hid = false;
    for (int i = 0; i < fields.num_uuids16; i++) {
        if (ble_uuid_u16(&fields.uuids16[i].u) == BLE_HID_SVC_UUID) { is_hid = true; break; }
    }
    // Some keyboards omit the service UUID from the ADV and only expose a HID appearance
    // (0x03C0 generic HID, 0x03C1 keyboard, 0x03C2 mouse). Accept those too.
    if (!is_hid && fields.appearance_is_present &&
        (appearance & 0xFFC0) == 0x03C0) {
        is_hid = true;
    }
    // Many BLE keyboards (e.g. "XECH Typewriter") advertise NO HID UUID / appearance in the ADV —
    // the HID service only appears after connecting. So surface EVERY named device and let the user
    // pick theirs from the scan list; esp_hidh confirms HID via GATT discovery on connect (a
    // non-HID pick simply fails to open). HID-UUID / HID-appearance devices are still accepted too.
...
static esp_err_t start_nimble_scan(uint32_t seconds)
{
    ...
    /* Ensure the host has an identity address (adopt the controller's public addr,
     * else a random one). Without this, ble_hs_id_infer_auto returns BLE_HS_ENOADDR
     * (rc=21) and every scan finds nothing. */
    rc = ble_hs_util_ensure_addr(0);
    if (rc != 0) {
        MODLOG_DFLT(ERROR, "ble_hs_util_ensure_addr failed; rc=%d\n", rc);
        return rc;
    }
