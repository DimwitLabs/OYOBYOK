...
int p_rename(const char *from, const char *to)
{
	...
	/* ESP-IDF's FAT rename() refuses to replace an existing target (POSIX
	 * rename is supposed to overwrite atomically). libgit2 relies on the
	 * overwrite for its write-lock-then-rename pattern — config, index, refs,
	 * and every commit. Remove the destination and retry. */
	p_unlink(to);
	if (!rename(from, to))
		return 0;
