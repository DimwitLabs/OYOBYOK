...
/* BYOK/ESP32: upstream is 65536, and these are used as STACK arrays (git_odb__hashfd,
 * blob.c, filter.c). A 64 KB frame on a 40 KB task stack jumped clean over the end-of-stack
 * canary/watchpoint and silently overwrote whatever sat below the git task's stack (WiFi TX
 * buffers, other tasks' TCBs, PSRAM heap headers) on EVERY working-tree hash (add/status).
 * 4 KB chunks hash just as well. Network I/O is heap-allocated and stays at its own size. */
#define GIT_BUFSIZE_DEFAULT 4096
#define GIT_BUFSIZE_FILEIO GIT_BUFSIZE_DEFAULT
#define GIT_BUFSIZE_FILTERIO GIT_BUFSIZE_DEFAULT
#define GIT_BUFSIZE_NETIO 65536   /* heap-allocated (smart transport); keep upstream size */
