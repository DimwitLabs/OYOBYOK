	...
	git_credential *cred)
{
	...
	if (rc == LIBSSH2_ERROR_PASSWORD_EXPIRED ||
		...
		/* BYOK: this is the branch that loops (cred_cb re-asked ~30x per sync). Log WHY. */
		char *msg = NULL; int len = 0;
		libssh2_session_last_error(session, &msg, &len, 0);
		printf("[byok-ssh] auth attempt failed rc=%d: %.*s\n", rc, len, msg ? msg : "");
		return GIT_EAUTH;
	}
...
static int load_known_hosts(LIBSSH2_KNOWNHOSTS **hosts, LIBSSH2_SESSION *session)
{
	...
	if ((error = git_sysdir_expand_homedir_file(&sshdir, SSH_DIR)) < 0 ||
		...
		/* OYOBYOK: there is no home dir on the device, so ~/.ssh can't be
		 * resolved. Treat that exactly like a missing known_hosts file — no
		 * hosts loaded, host trust is decided by the certificate_check callback
...
static LIBSSH2_ALLOC_FUNC(byok_ssh_alloc)
{
	void *p = heap_caps_malloc(count, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
	(void)abstract;
	return p ? p : malloc(count);
}
static LIBSSH2_REALLOC_FUNC(byok_ssh_realloc)
{
	void *p = heap_caps_realloc(ptr, count, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
	(void)abstract;
	return p ? p : realloc(ptr, count);
}
static LIBSSH2_FREE_FUNC(byok_ssh_free)
{
	(void)abstract;
	free(ptr);
}
	...
	git_stream *io)
{
	...
	/* BYOK: libssh2's buffers (packet/crypto working memory) go to PSRAM so they don't compete
	 * with WiFi for internal DMA RAM; LWIP copies our send() data into its own internal pbufs, so
	 * PSRAM here is safe. Internal fallback if PSRAM is exhausted. */
	s = libssh2_session_init_ex(byok_ssh_alloc, byok_ssh_free, byok_ssh_realloc, NULL);
	if (!s) {
	...
	/* known_hosts may be NULL on this device (no home dir / no known_hosts file);
	 * skip the preference probe in that case — an empty prefs lets the handshake
	 * negotiate the hostkey type normally. Passing NULL into libssh2_knownhost_checkp
	 * crashes (LoadProhibited). Host trust is handled via the certificate_check cb. */
	if (known_hosts)
