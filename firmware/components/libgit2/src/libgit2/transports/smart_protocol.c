	...
	const git_fetch_negotiation *wants)
{
	...
	if (wants->depth > 0) {
		...
		/*
		 * The wants (with deepen) were already sent above to negotiate
		 * shallow roots. Clear the send buffer so they are NOT re-sent
		 * together with the later "done": a duplicate want block makes
		 * some servers (GitHub over SSH) close the connection, which
		 * surfaces as "could not read from remote repository". For a
		 * non-shallow fetch this block is skipped and data is sent once.
		 */
		git_str_clear(&data);
	}
