...
/* BYOK: drop the cached tree extension so the next write_tree rebuilds and actually WRITES
 * every subtree object. A stale/partial TREE cache (e.g. persisted across earlier failed push
 * attempts) can hold a "valid" subtree oid whose object was never written to the ODB; write_tree
 * then trusts that oid (tree.c write_tree: entry_count>=0 -> copy cache->oid) and fails validation
 * with "invalid object specified". Clearing the cache forces a correct, self-contained rebuild. */
void git_index__drop_tree_cache(git_index *index)
{
	if (index) {
		/* the cache lives in tree_pool (see index reset paths); clear both. */
		index->tree = NULL;
		git_pool_clear(&index->tree_pool);
	}
}
